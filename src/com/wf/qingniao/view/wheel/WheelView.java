package com.wf.qingniao.view.wheel;
...
public class WheelView extends View {
    ...
    private static final int ITEM_OFFSET_PERCENT = 1;
    ...
    // Center Line
    private Drawable centerDrawable;// 中间�?2根线的颜�?
    ...
    public void setScrollingPerformed(boolean isScrollingPerformed) {
        this.isScrollingPerformed = isScrollingPerformed;
    }
    ...
    WheelScroller.ScrollingListener scrollingListener = new WheelScroller.ScrollingListener()
    {
        ...
        public void onJustify() {
            if (Math.abs(scrollingOffset) > WheelScroller.MIN_DELTA_FOR_SCROLLING) {
                scroller.scroll(scrollingOffset, 0); // 滚动
            }
        }
    };
    ...
    public boolean popuwindowIsShow = false;

    void showTimechangeRemindDialog() {
        if (popuwindowIsShow == true) {
            return;
        } else {

            // showConfirmPopuWindow(context);
            popuwindowIsShow = true;
        }
    ...
    Handler handle = new Handler()
    {
        ...
        public void handleMessage(Message msg) {
            if (msg.what == 0x101) { // 设置为第�?次才弹窗
                showTimechangeRemindDialog();
            } else {
     ...
     *            the scrolling value
     */
    private void doScroll(int delta) { // 滚动距离
    ...
    private boolean rebuildItems() {
        ...
        if (!updated && range != null) {
            updated = firstItem != range.getFirst() || (itemsLayout != null && itemsLayout.getChildCount() != range.getCount());
        }

        if (range != null && firstItem > range.getFirst() && firstItem <= range.getLast()) {
